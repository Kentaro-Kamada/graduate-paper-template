...
## 3.8　フォント
　どのフォントを使うかは，preambleで指定する。luatexja-fontspecというパッケージを使い，\\setmainfontで英数字，\\setmainjfontで日本語のフォントを設定できる。なお，本稿はMS明朝 + Times New Romanで作成している。詳しくは公式vignette(http://mirrors.ibiblio.org/CTAN/macros/luatex/generic/luatexja/doc/luatexja-ja.pdf)を参照。
　
